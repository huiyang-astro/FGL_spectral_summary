...
for id in ids:
    if n_comp ==1:
        ...
        if id==1:
            guess(model_comp)
        set_par(model_comp.norm, val=1., min=1e-15, max=1e3)
        priors += [bxa.create_loguniform_prior_for(model_comp.norm)]
        # priors += [bxa.create_uniform_prior_for(model_comp.norm)]
        parameters += [model_comp.norm]
        if id >1:
            if model == 'mekal':
        ...
        if id==1:
            guess(model_comp1)
        set_par(model_comp1.norm, val=1., min=1e-15, max=1e3)
        priors += [bxa.create_loguniform_prior_for(model_comp1.norm)]
        # priors += [bxa.create_uniform_prior_for(model_comp.norm)]
        parameters += [model_comp1.norm]
        model_comp2 = get_model_component(model_dict[model]['model_prefix2'] + str(id))
        if id==1:
            guess(model_comp2)
        set_par(model_comp2.norm, val=1., min=1e-15, max=1e3)
        priors += [bxa.create_loguniform_prior_for(model_comp2.norm)]
        # priors += [bxa.create_uniform_prior_for(model_comp.norm)]
        parameters += [model_comp2.norm]
        if id >1:
            if model == 'twomekal':
...
if model == 'mekal':
    set_par(mek1.kT, val=0.3, min=0.1, max=50)
    priors += [bxa.create_loguniform_prior_for(mek1.kT)]
    parameters += [mek1.kT]
if model == 'powerlaw':
    set_par(pl1.PhoIndex, val=2, min=-3, max=10)
    priors += [bxa.create_uniform_prior_for(pl1.PhoIndex)]
    parameters += [pl1.PhoIndex]
if model == 'bb':
    set_par(bb1.kT, val=0.3, min=0.0001, max=10)
    priors += [bxa.create_loguniform_prior_for(bb1.kT)]
    parameters += [bb1.kT]
if model == 'twomekal':
    set_par(mek11.kT, val=0.3, min=0.1, max=50)
    priors += [bxa.create_loguniform_prior_for(mek11.kT)]
    parameters += [mek11.kT]
    set_par(mek21.kT, val=0.3, min=0.1, max=50)
    priors += [bxa.create_loguniform_prior_for(mek21.kT)]
    parameters += [mek21.kT]
if model == 'powerlawbb':
    set_par(pl1.PhoIndex, val=2, min=-3, max=10)
    priors += [bxa.create_uniform_prior_for(pl1.PhoIndex)]
...
# srcnh = Parameter('src1', 'nH', 21, 19, 26, 19, 26)
# tbabs1.nh = 10**(srcnh - 22)
set_par(tbabs1.nH, val=0.1, min=1.e-2, max=1.e4)
parameters += [tbabs1.nh]
priors += [bxa.create_loguniform_prior_for(tbabs1.nH)]
...
for id in ids:
    ...
    ylo, ymid, yhi = numpy.percentile(ypreds, [15.87, 50, 84.13], axis=0)
    numpy.savetxt(prefix + '/src_subtract_'+str(id)+'.txt.gz', numpy.transpose([m.dataplot.x, m.dataplot.y, m.dataplot.yerr, m.modelplot.x, ylo, ymid, yhi]))
